...
class GUIAnalyzer(val nbt: CompoundTag): Screen(NarratorChatListener.NO_TITLE) {
    ...
    override fun init() {
        super.init()
        this.effNextButton = addRenderableWidget(Button(width / 2 - 60, 186, 30, 20, TranslationUtils.getTranslatedTextComponent("analyzer", "page", "prev")) { button ->
            effPage = if(++effPage >= data.size) 0 else effPage
            genePage = 0
        })
        this.effPrevButton = addRenderableWidget(Button(width / 2 + 30, 186, 30, 20, TranslationUtils.getTranslatedTextComponent("analyzer", "page", "next")) { button ->
    ...
    override fun render(stack: PoseStack, x: Int, y: Int, ticks: Float) {
        ...
        drawCenteredString(stack, mc.font, TranslationUtils.getTranslatedText("analyzer", "page", "analyzer"), width / 2, 17, 0xffffff)
        if(effPage in data.indices){
            drawString(stack, mc.font, TranslationUtils.getTranslatedText("analyzer", "page", "effect"), (width - 122) / 2, 31, 0xffffff)
            ...
            if(data.genes.isNotEmpty()){
                drawString(stack, mc.font, TranslationUtils.getTranslatedText("analyzer", "page", "genes"), (width - 122) / 2, 55, 0xffffff)
                ...
                drawCenteredString(stack, mc.font, TranslationUtils.getTranslatedText("analyzer", "page", "genes_empty"), width / 2, 67, 0xffffff)
        }else
            drawCenteredString(stack, mc.font, TranslationUtils.getTranslatedText("analyzer", "page", "effects_empty"), width / 2, 37, 0xffffff)
