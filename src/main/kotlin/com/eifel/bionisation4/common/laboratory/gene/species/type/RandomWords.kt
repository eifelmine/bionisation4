...
class RandomWords(): Gene(InternalConstants.GENE_RANDOM_WORDS_ID, "Random words", true) {
    ...
    override fun perform(entity: LivingEntity, effect: AbstractEffect) {
        ...
        if(entity is ServerPlayer && entity.getBioTicker() % delay == 0){
            ...
            ServerLifecycleHooks.getCurrentServer().playerList.players.forEach { it.sendChatMessage(
                OutgoingPlayerChatMessage.create(PlayerChatMessage.unsigned(MessageSigner.create(it.uuid), ChatMessageContent("§6${text.joinToString(" ")}"))), true, ChatType.bind(ChatType.CHAT, it)) }
        }
    }
